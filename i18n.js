...
(function () {
  ...
  var DICT = {
    ...
    "theme.switchToLight": { uz: "Yorug' rejim", en: "Light mode", ru: "Светлый режим" },
    "theme.switchToDark": { uz: "Qorong'i rejim", en: "Dark mode", ru: "Тёмный режим" },
  ...
  // Sahifa chizilishidan OLDIN mavzu qo'llanadi — miltillash (flash) bo'lmasligi uchun.
  ...
  function setTheme(theme) {
    ...
    updateThemeBtnUI();
    try {
      document.dispatchEvent(new CustomEvent("beks:themechange", { detail: { theme: currentTheme } }));
    } catch (e) {}
  }
  ...
  function buildSwitcher() {
    ...
    style.textContent =
      ".beksLangSwitch{position:fixed;top:16px;right:16px;z-index:3000;display:flex;align-items:center;gap:8px;font-family:Inter,'Segoe UI',Arial,sans-serif;}" +
      ".beksLangBtn,.beksThemeBtn{display:flex;align-items:center;gap:6px;min-height:38px;padding:7px 12px;border-radius:8px;" +
        "border:1px solid var(--line2,rgba(255,255,255,.16));background:var(--panel,#0a1424);" +
        "color:var(--text,#f7f9fc);font-size:12.5px;font-weight:800;cursor:pointer;box-shadow:var(--shadowSmall,0 10px 25px rgba(0,0,0,.25));}" +
      ".beksThemeBtn{padding:7px 10px;font-size:16px;line-height:1;}" +
      ".beksLangBtn:hover,.beksThemeBtn:hover{border-color:var(--cyan,#20d9ff);background:rgba(32,217,255,.14);}" +
      ".beksLangMenu{position:absolute;top:calc(100% + 8px);right:0;min-width:150px;padding:6px;border-radius:8px;" +
        "border:1px solid var(--line2,rgba(255,255,255,.16));background:var(--panel,#09121f);" +
        "box-shadow:var(--shadow,0 18px 40px rgba(0,0,0,.35));display:none;flex-direction:column;gap:2px;}" +
      ".beksLangMenu.show{display:flex;}" +
      ".beksLangOpt{display:flex;align-items:center;gap:8px;padding:8px 10px;border-radius:8px;border:none;" +
        "background:transparent;color:var(--text,#f7f9fc);font-size:12.5px;font-weight:700;text-align:left;cursor:pointer;}" +
      ".beksLangOpt:hover{background:rgba(127,127,127,.14);}" +
      ".beksLangOpt.active{background:rgba(32,217,255,.16);color:var(--cyan,#20d9ff);}" +
      "@media(max-width:560px){.beksLangSwitch{top:10px;right:10px;gap:6px;}.beksLangBtn,.beksThemeBtn{min-height:34px;padding:6px 10px;font-size:11.5px;}.beksThemeBtn{padding:6px 9px;font-size:15px;}}";
    ...
    themeBtn.className = "beksThemeBtn";
    themeBtn.id = "beksThemeBtn";
    themeBtn.addEventListener("click", function (e) {
      e.stopPropagation();
      toggleTheme();
    });
    switcherRoot.appendChild(themeBtn);
  ...
  function updateThemeBtnUI() {
    var btn = document.getElementById("beksThemeBtn");
    if (btn) {
      btn.textContent = currentTheme === "dark" ? "☀️" : "🌙";
      btn.title = currentTheme === "dark"
        ? t("theme.switchToLight", "Yorug' rejim")
        : t("theme.switchToDark", "Qorong'i rejim");
    }
  }
  ...
  window.getAppTheme = function () { return currentTheme; };
